...
class BrmLogger(logger: org.slf4j.Logger) extends Logger(logger) {
  ...
  def info(uuid: UUID, className: String, methodName: String, message: String): Unit =
  ...
  def error(className: String, methodName: String, message: String): Unit =
    logger.error(s"[$BRM_KEY:${KeyHolder.getKey()}],[$className][$methodName] : $message")

  def debug(className: String, methodName: String, message: String): Unit =
    logger.debug(s"[$BRM_KEY:${KeyHolder.getKey()}],[$className][$methodName] : $message")
